...
__MUITOS_CARACTERES_POR_LINHA__ = 20
...
def preprocess_image(image_path, erosion_iterations):
    print("Tentando carregar a imagem de:", image_path)
    
    # 1. Carrega a imagem em BGR
    ...
    # 3. Binariza (Otsu) -> texto branco em fundo preto
    # Se seu texto for claro em fundo escuro, troque o tipo para THRESH_BINARY_INV
    _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    
    # 4. Erosão
    #    - kernel define a vizinhança que será "corroída"
    #    - iterations=1 -> faz uma erosão leve
    #    - iterations=2 -> faz a erosão duas vezes
    kernel = np.ones((2,2), np.uint8)  # pode ajustar o tamanho
...
def ocr_with_confidence_filter(image_path, min_confidence=50, erosion_iterations=1, scale_factor=1.5):
    """
    Realiza OCR com filtros de pré-processamento e filtro de confiança.
    Retorna o texto extraído com confiança acima de min_confidence.
    
    Parâmetros:
        - image_path: Caminho da imagem a ser processada.
        - min_confidence: Valor mínimo de confiança para considerar uma palavra. Default: 50.
        - erosion_iterations: Número de iterações de erosão para melhorar a identação. Default: 1.
        - zoom_factor: Fator de zoom para aumentar a resolução. Default: 1.5.
    
    Retorna:
        O texto extraído com confiança acima de min_confidence.
    """
    preprocessed_img = preprocess_image(image_path, erosion_iterations)
    
    # 1) Upscale
    img_upscaled = upscale_image(preprocessed_img, scale_factor)
...
def remover_linhas_extras(texto: str) -> list:
    ...
    # Splita por \n ou por " " com 3 ou mais espaços
    linhas = re.split(r'\n| {3,}', normalize_words(texto))
...
def most_likely_name(nomes_lidos, nomes_possiveis: list, threshold: int = 80) -> str:
    """
    Retorna o nome mais provável de uma lista de nomes possíveis, baseado em fuzzy matching.
    """
    best_match = ""
    
    if type(nomes_lidos) == str:
        nomes_lidos = [nomes_lidos]
...
def extrair_nome_destinatario_v1(texto: str) -> str:
    ...
    for linha in linhas_filtradas:
        # se tiver até 2 palavras, não é um nome
...
if __name__ == "__main__":
    ...
    # file_debug = "WhatsApp Image 2025-03-07 at 10.36.23.jpeg"
    # file_debug = "WhatsApp Image 2025-03-06 at 10.16.49.jpeg"
    file_debug = "Debug4.png"
    # file_debug = "WhatsApp Image 2025-03-06 at 10.17.02.jpeg"
    ...
    for file_name in os.listdir(images_folder):
        ...
        if file_name.lower().endswith(('.jpg', '.jpeg', '.png')):
            ...
            try:
                ...
                min_confidences = [60, 40, 20, 10, 0]
                for min_conf in min_confidences:
                    print(f"{"":-^100}")
                    print_frase = f'--- Tentando com confiança mínima de {min_conf} ---'
                    print(f"\n\n{print_frase:^100}\n\n")
                    
                    
                    texto_cru = ocr_with_confidence_filter(image_path, min_conf, erosion_iterations=1)
                    print("Texto OCR Filtrado:\n")
                    print(texto_cru)
                    print("\n\n")
                    
                    ## Correções na leitura
                    # 1. Erosão: Melhorar a identação
                    # Se a média de caracteres por linha for muito alta, tenta com mais erosões
                    linhas = texto_cru.split('\n')
                    media_char_linha = sum([len(linha) for linha in linhas]) / len(linhas)
                    if media_char_linha >= 20:
                        print_frase = f'Aplicando Erosão Adicional'
                        print(f"\n\n{print_frase:^100}\n\n")
                        texto_cru = ocr_with_confidence_filter(image_path, min_conf, erosion_iterations=2)
                        
                        print("Texto OCR Filtrado:\n")
                        print(texto_cru)
                    
                    
                    # 2. Zoom: Melhorar a resolução
                    # Se a quantidade de linhas for muito alta, tenta com zoom
                    if len(linhas) > 15:
                        print_frase = 'Aplicando Scaling Factor'
                        print(f"\n\n{print_frase:^100}\n\n")
                        texto_cru = ocr_with_confidence_filter(image_path, min_conf, scale_factor=2)
                ...
                if not nome_dest:
                    nome_dest = input("\n\n\nNão foi possível extrair o nome automaticamente. Por favor, insira manualmente: ")
